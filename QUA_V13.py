...
# Estilos CSS personalizados para márgenes y distribución
st.markdown(
    """
    <style>
    .stApp {
...
# Función para generar la tabla y el gráfico de resultados
def generar_tabla_y_grafico(df_resultados):
    # Calcular la cantidad de llamadas por calificación
    conteo_calificaciones = df_resultados['Rating'].value_counts().sort_index()

    # Calcular los porcentajes
    porcentajes = (conteo_calificaciones / conteo_calificaciones.sum()) * 100

    # Crear un DataFrame para la tabla
    tabla = pd.DataFrame({
        'Notas': [5, 4, 3, 2, 1],
        'llamadas Monito': [conteo_calificaciones.get(5, 0), conteo_calificaciones.get(4, 0), conteo_calificaciones.get(3, 0), conteo_calificaciones.get(2, 0), conteo_calificaciones.get(1, 0)],
        'Total (%)': [f"{porcentajes.get(5, 0):.0f}%", f"{porcentajes.get(4, 0):.0f}%", f"{porcentajes.get(3, 0):.0f}%", f"{porcentajes.get(2, 0):.0f}%", f"{porcentajes.get(1, 0):.0f}%"]
    })

    # Calcular el total de llamadas y la nota promedio
    total_call_qua = conteo_calificaciones.sum()
    nota_promedio = df_resultados['Rating'].mean()

    # Mostrar la tabla en Streamlit
    st.write("Distribución de Calificaciones:")
    st.dataframe(tabla)

    # Mostrar el total de llamadas y la nota promedio
    st.write(f"Total Call Qua: {total_call_qua}")
    st.write(f"Nota Promedio: {nota_promedio:.2f}")

    # Generar el gráfico de barras
    fig, ax = plt.subplots(figsize=(10, 6))
    ax.bar(tabla['Notas'], tabla['llamadas Monito'], color='skyblue')
    ax.set_xlabel('Notas')
            ...
            if not df.empty:
                ...
                if selected_call_ids:
                    ...
                    if resultados:
                        ...
                        # Mostrar la tabla y el gráfico de resultados
                        generar_tabla_y_grafico(df_resultados)
