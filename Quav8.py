...
def generar_grafico_semaforizado(promedio_calificaciones):
    if promedio_calificaciones >= 4:
        ...
        estado_semaforo = "Muy Bueno"
    elif 2.5 <= promedio_calificaciones < 4:
        color_semaforo = "yellow"
        estado_semaforo = "Intermedio"
    else:
        color_semaforo = "red"
        estado_semaforo = "Bajo"
...
def generar_informe_pdf(resultados, promedio_calificaciones):
    ...
    for resultado in resultados:
        ...
        pdf.multi_cell(0, 10, txt=f"Análisis: {resultado['Analysis']}")
        pdf.ln(5)
    pdf.output("informe_llamadas.pdf")
    return "informe_llamadas.pdf"

# Interfaz de usuario en Streamlit
def main():
    st.sidebar.title("Configuración")
    hubspot_token = st.sidebar.text_input("Clave de API de HubSpot:", type="password")
    google_api_key = st.sidebar.text_input("Clave de API de Google:", type="password")
    ...
    if hubspot_token and google_api_key:
        ...
        fecha_desde = st.sidebar.date_input("Fecha de inicio:")
        fecha_hasta = st.sidebar.date_input("Fecha de fin:")
