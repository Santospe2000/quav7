...
# Función para generar el gráfico de barras con línea de tendencia
def generar_grafico_barras_con_linea_tendencia(df_resultados):
    # Ordenar los resultados por la calificación de mayor a menor
    df_resultados = df_resultados.sort_values(by='Rating', ascending=False)
    
    # Crear el gráfico de barras
    fig, ax = plt.subplots(figsize=(10, 6))
    
    # Graficar las barras
    ax.bar(df_resultados['Call ID'], df_resultados['Rating'], color='skyblue', label='Calificación')
    
    # Calcular la línea de tendencia
    z = np.polyfit(range(len(df_resultados)), df_resultados['Rating'], 1)
    p = np.poly1d(z)
    ax.plot(df_resultados['Call ID'], p(range(len(df_resultados))), "r--", label='Línea de Tendencia')
    
    # Configuraciones adicionales del gráfico
    ax.set_xlabel('ID de Llamada')
    ax.set_ylabel('Calificación')
    ax.set_title('Calificaciones de Llamadas con Línea de Tendencia')
    ax.legend()
    ax.grid(True)
            ...
            if not df.empty:
                ...
                if selected_call_ids:
                    ...
                    if resultados:
                        ...
                        # Mostrar el gráfico de barras con línea de tendencia
                        generar_grafico_barras_con_linea_tendencia(df_resultados)
